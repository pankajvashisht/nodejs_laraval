...
class UserController extends ApiController {
    ...
    async addUser(req, res) {
        try {
            ...
            app.success(res , {message:"User signup successfully", data:{
    ...
    async loginUser(req, res) {
        try {
            ...
            let login_details = await DB.find('users', 'first', {
                ...
                fields:['id', 'name', 'password', 'email', 'authorization_key', 'gender', 'social_id', 'social_token', 'interest', 'profile']
            });
            if (login_details) {
                if(request_data.password != login_details.password)  throw "Wrong Email or password"; 
    ...
    async allUsers(req, res){
        try {
            ...
            let all_users = await DB.find('users', 'all', {
                conditions:{
                    1 : ' > (select count(id) from likes where user_id = '+req.body.user_id+' and friend_id = users.id)',
                    gender : req.body.userInfo.interest
                },
                fields:['id', 'name', 'email', 'gender', 'social_id', 'social_token', 'interest', 'profile']
            });
            app.success(res , {
